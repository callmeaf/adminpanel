...
interface ICoinsTableProps {
  ...
  onFilter: TOnFilter;
  loading: boolean;
  onDelete: TOnDelete<ICoinModel> | TOnForceDelete<ICoinModel>;
  loadingDelete: boolean;
  onRestore: TOnRestore<ICoinModel>;
  loadingRestore: boolean;
  onEdit: TOnEdit<ICoinModel>;
  onStatusUpdate: TOnUpdate<ICoinModel>;
  onTypeUpdate: TOnUpdate<ICoinModel>;
  onExport: TOnExport;
}

const CoinsTable: React.FC<ICoinsTableProps> = ({
  tableId,
  trashed,
  coins,
  paginate,
  ...
  const t = useTranslations("Coin.Widgets.Table");
        ...
        <Show.Else>
          {coins.map((coin, index) => (
            <TableRow key={coin.id}>
              ...
              <TableCell>{coin.createdAtText}</TableCell>
              <TableCell width={300}>
                <TableAction
                  model={coin}
